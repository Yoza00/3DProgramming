...
bool Application::Init(int w, int h)
{
	...
	// カメラ初期化
	//===================================================================
	std::shared_ptr<TPSCamera> _spCamera =
		std::make_shared<TPSCamera>();
	_spCamera->Init();
	_spCamera->SetTarget(_character);		// カメラのターゲットを設定
	_spCamera->ResistHitObject(_terrain);	// 事前に当たり判定するオブジェクトのポインタを当たり判定用のリストに格納しておく
	_character->SetCamera(_spCamera);
	m_GameObjectList.push_back(_spCamera);
	// カメラの更新は一番最後でよい(現状は)
