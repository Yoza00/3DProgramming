...
// ここからステートパターン関係
void Character::ChangeActionState(std::shared_ptr<ActionStateBase>	_nextState)
{
...
// ↓待機状態！
void Character::ActionIdle::Enter(Character& _owner)
{

}


void Character::ActionIdle::Update(Character& _owner)
{
	if (GetAsyncKeyState(VK_SPACE) & 0x8000)
	{
		_owner.ChangeActionState(std::make_shared<ActionJump>());
		return;
	}
}

void Character::ActionIdle::Exit(Character& _owner)
{
	
}

// ↓ジャンプ状態！
void Character::ActionJump::Enter(Character& _owner)
{
	_owner.m_Gravity = -0.5f;
}


void Character::ActionJump::Update(Character& _owner)
{
...
void Character::ActionJump::Exit(Character& _owner)
{

}

// ↓歩行状態！
void Character::ActionWalk::Enter(Character& _owner)
{
	
}


void Character::ActionWalk::Update(Character& _owner)
{
	float			_moveSpd = 0.05f;
	Math::Vector3	_nowPos = _owner.GetPos();
	...
	// キャラクターの回転行列を創る
	_owner.UpdateRotate(_moveVec);

	// キャラクターのワールド行列を創る処理
	Math::Matrix _rotation = Math::Matrix::CreateRotationY(DirectX::XMConvertToRadians(_owner.m_worldRot.y));
	_owner.m_mWorld = _rotation * Math::Matrix::CreateTranslation(_nowPos);
}

void Character::ActionWalk::Exit(Character& _owner)
{
