...
void KdLessonShader::Begin()
{
    ...
    if (KdShaderManager::Instance().SetVertexShader(m_VS))
    {
		...
		// CPU側(こちら)からGPU側に知らせる必要がある
		// 第一引数に、保存先、第二引数にモデルの行列情報を保存しようとしていることを知らせているイメージ
...
#include "KdLessonShader_PS.shaderInc"
   ...
   // 定数バッファ
   //-------------------------------------
	m_cb1_Mesh.Create();		// 用意した箱を組み立てる
	m_cb2_Material.Create();	// 用意した箱を組み立てる
...
void KdLessonShader::Release()
{
	...
	m_cb1_Mesh.Release();		// 用意した箱を解放する
	m_cb2_Material.Release();	// 用意した箱を解放する
}
